...
namespace
{
    ...
    constexpr const char *NVIDIA_CHAT_URL = "https://integrate.api.nvidia.com/v1/chat/completions";
    ...
    json messageToNvidia(const Message &message)
    {
        json payload;
        ...
        if (!message.images.empty())
        {
            ...
            for (const auto &image_base64 : message.images)
            {
                parts.push_back({{"type", "image_url"},
                                 {"image_url", {{"url", std::string("data:image/*;base64,") + image_base64}}}});
            }
    ...
    Message NvidiaToMessage(const json &response_message)
    {
        Message reply;
    ...
    json buildPayload(const std::string &model, const std::vector<Message> &history)
    {
        ...
        {
            json message = messageToNvidia(msg);
...
namespace NvidiaClient
{
    ...
    bool chat(const std::string &model, const std::vector<Message> &history, Message &reply, std::string &err)
    {
        ...
        const char *api_key = std::getenv("NVIDIA_API_KEY");
        ...
        curl_easy_setopt(curl, CURLOPT_URL, NVIDIA_CHAT_URL);
